...
class FamilyDAO:
    ...
    async def get_family_with_contributions(self, db: AsyncSession,fam_id:int) -> Family | None: 
        stmt = (
            select(self.model).options(selectinload(Family.contributions),with_loader_criteria(FamilyContribution, FamilyContribution.fcon_is_deleted == False))
    ...
    async def update_family_contribution(self, db: AsyncSession, user_id: int,data: FamilContributionUpdateSchema) -> FamilyContribution | None:
        ...
        q = select(FamilyContribution).where(FamilyContribution.fcon_id == data.fcon_id)
        res = await db.execute(q)
        ...
        new_amount = data.fcon_amount
        delta = new_amount - old_amount

        # 2. Update FamilyContribution record
        stmt = (
            update(FamilyContribution)
            .where(FamilyContribution.fcon_id == data.fcon_id)
            .values(
